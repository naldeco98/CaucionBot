...
Automatizar la colocación de "cauciones bursátiles" (préstamos con garantía de títulos) en el mercado financiero argentino utilizando la API de **InvertirOnline (IOL)**. El objetivo es reinvertir el saldo líquido diariamente para aprovechar el interés compuesto.
...
- `src/iol_client.py`: Wrapper sobre la API de IOL (utilizando la librería `iol-api`). Maneja autenticación y operaciones bursátiles.
...
3. **Asincronía**: La API de IOL y Telegram se manejan de forma asincrónica (`asyncio`). Mantener este patrón en todas las extensiones.
...
1. Autenticación en IOL.
2. Consulta de saldo líquido disponible en la cuenta.
...
## Dependencias Clave
- `iol-api`: Interacción con el broker.
